...
public class GetProductByIdHandler {
    ...
    public Optional<Product> execute(UUID id, Optional<String> ifNoneMatch) {

        if (ifNoneMatch.isPresent()) {
            Optional<Product> fromRedis = strategies.get("redis").execute(id, ifNoneMatch);
            if (fromRedis.isPresent()) {
                return fromRedis;
            }
        }

        return strategies.get("db").execute(id, ifNoneMatch);
    }
}
